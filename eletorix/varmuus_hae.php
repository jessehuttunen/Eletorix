...
$stmt = $db->prepare("SELECT * FROM ilmoitus INNER JOIN kategoria ON ilmoitus.kategoria_idkategoria=kategoria.idkategoria INNER JOIN kayttaja ON ilmoitus.kayttaja_idkayttaja=kayttaja.idkayttaja WHERE tuotteennimi like CASE WHEN :tuotteennimi IS NULL THEN tuotteennimi  ELSE  :tuotteennimi2 END AND idkategoria= CASE WHEN :kategoria IS NULL THEN idkategoria ELSE :kategoria2 END AND kaupunki like CASE WHEN :kaupunki IS NULL THEN kaupunki ELSE :kaupunki2 END AND hinta <= CASE WHEN :m_hinta IS NULL THEN hinta ELSE :m_hinta2 END AND hinta >= CASE WHEN :p_hinta IS NULL THEN hinta ELSE :p_hinta2 END order by ilmoitus.idilmoitukset desc limit $montaIlmoitustaNaytetaan;");
